# Updated Code to Fix Cell ID Extraction Issue

The issue is that the LLM is **hallucinating/fabricating cell IDs** instead of extracting them from the actual context. The response shows fake cells like "ATL00123A11" instead of real ones like "B9BH0003A21".
...
### 1. **Added Cell Name Validation Functions**
```python
def validate_nokia_cell_name(cell_name: str) -> bool:
    """Validate if a cell name follows Nokia naming convention."""
...
### 2. **New Multi-Metric Query Prompt**
The prompt now:
- **Explicitly forbids fabrication** of cell IDs
- **Explains Nokia naming convention** in detail
- **Provides fallback response** when no data is found
- **Requires verification** that cells exist in context

### 3. **Hallucination Detection**
```python
# Known fake patterns (generic patterns that don't match real Nokia cells)
fake_patterns = [
    r'^[A-Z]{3}\d{5}[A-Z]\d{2}$',  # Pattern like ATL00123A11
    r'^(ATL|CHI|DAL|DET|HOU|IND|MIA|PHX|SEA|TAM)\d+',  # Airport codes
]
```

### 4. **Correct SITEID Derivation**
```python
if len(cell_name) == 11:
    # B9BH0003A21 → site_id=9BH0003A, sector_name=9BH0003A21
    sector_name = cell_name[1:]  # Remove prefix
    site_id = sector_name[:8]     # First 8 chars
```

### 5. **Temperature Set to 0**
```python
llm = ChatOpenAI(
...
When fabricated data is detected, the response includes:
```
⚠️ WARNING: All cell IDs appear to be fabricated/hallucinated. 
No matching cell data found in the database.
```
